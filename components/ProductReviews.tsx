...
const ProductReviews: React.FC<ProductReviewsProps> = ({ productId, productName }) => {
    ...
    return (
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            {star <= rating ? (
              <StarIcon className={`${sizeClass} text-[#8A4B24]`} />
            ) : (
              <StarOutlineIcon className={`${sizeClass} text-gray-300 ${interactive ? 'hover:text-[#8A4B24]/50' : ''} transition-colors`} />
            )}
          </button>
        ))}
      </div>
  ...
  return (
    <section className="bg-white rounded-sm border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <MessageSquare className="w-5 h-5 text-[#8A4B24]" />
              ...
              {[5, 4, 3, 2, 1].map((star) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2">
                    ...
                    <StarIcon className="w-3 h-3 text-[#8A4B24] flex-shrink-0" />
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#8A4B24] rounded-full transition-all duration-500"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-10 py-8">
        ...
        <div className="mb-8 flex justify-center">
          <button
            ...
            className={`inline-flex items-center gap-2 px-7 py-3 rounded-sm font-semibold text-sm transition-all duration-300 ${
              showForm
                ? 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                : 'bg-[#8A4B24] text-white hover:bg-[#6E3A1B] shadow-md hover:shadow-lg hover:shadow-[#E0D5C4] hover:-translate-y-0.5 active:translate-y-0'
            }`}
          >
            {showForm ? (
        ...
        {showForm && (
          <form
            ...
            className="mb-10 p-6 md:p-8 bg-gray-50 rounded-sm border border-gray-100 space-y-6"
          >
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="w-4 h-4 text-[#8A4B24]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">
                  Name <span className="text-[#8A4B24]">*</span>
                </label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-sm focus:border-[#8A4B24] focus:outline-none focus:ring-2 focus:ring-[#8A4B24]/10 text-sm transition-all"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wider">
                Rating <span className="text-[#8A4B24]">*</span>
              </label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">
                Your Review <span className="text-[#8A4B24]">*</span>
              </label>
              <textarea
                required
                ...
                rows={4}
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-sm focus:border-[#8A4B24] focus:outline-none focus:ring-2 focus:ring-[#8A4B24]/10 text-sm resize-none transition-all"
            ...
            <button
              type="submit"
              disabled={submitting}
              className={`w-full py-3.5 rounded-sm text-sm font-bold uppercase tracking-wide transition-all duration-300 flex items-center justify-center gap-2 ${
                submitting
                  ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                  : 'bg-[#8A4B24] text-white hover:bg-[#6E3A1B] shadow-md hover:shadow-lg hover:shadow-[#E0D5C4]'
              }`}
            >
              {submitting ? (
        ...
        {loading ? (
          <div className="py-16 text-center">
            <div className="relative w-12 h-12 mx-auto mb-4">
              <div className="absolute inset-0 rounded-full border-2 border-[#8A4B24] border-t-transparent animate-spin" />
              <ShoppingBag className="absolute inset-0 m-auto w-5 h-5 text-[#8A4B24]" />
            </div>
            <p className="text-gray-500 text-sm">Loading reviews...</p>
          </div>
        ) : reviews.length === 0 ? (
          <div className="py-16 text-center border-2 border-dashed border-gray-100 rounded-sm">
            <div className="w-16 h-16 bg-[#F3EBE0] rounded-full flex items-center justify-center mx-auto mb-4">
              <MessageSquare className="w-8 h-8 text-[#8A4B24]/40" />
            </div>
            <p className="text-gray-900 font-semibold mb-1">No Reviews Yet</p>
            <p className="text-gray-500 text-sm">
              Be the first to review{' '}
              <span className="text-[#8A4B24] font-medium">{productName}</span>
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {reviews.map((r) => (
              <div
                key={r.id}
                className="p-5 md:p-6 bg-white border border-gray-100 rounded-sm hover:border-[#8A4B24]/20 hover:shadow-sm transition-all duration-300"
              >
                {/* Reviewer info */}
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#8A4B24] to-[#A8653A] flex items-center justify-center flex-shrink-0 shadow-md shadow-[#E0D5C4]">
                  ...
                  <div className="mt-4 flex gap-2 flex-wrap ml-13">
                    {r.images.map((src, i) => (
                      <button
                        ...
                        className="relative group/img overflow-hidden rounded-sm border-2 border-gray-100 hover:border-[#8A4B24]/40 transition-all"
                      >
                        <img
                          src={src}
      ...
      {lightboxSrc && (
        ...
        >
          ...
          <img
            src={lightboxSrc}
            alt="Review"
            className="max-w-full max-h-[85vh] rounded-sm object-contain shadow-2xl"
